...
from common.model import GlossaryTerm, GlossaryTermUpdate
...
@router.get('/', response_model=List[GlossaryTerm], summary='Получить все термины')
def read_terms(db: SessionDep, skip: int = 0, limit: int = 100):
	terms = crud.get_terms(db, skip=skip, limit=limit)
	return terms

@router.get('/keyword/{keyword}', response_model=GlossaryTerm, summary='Получить термин по ключевому слову')
...
@router.post('/', response_model=GlossaryTerm, status_code=status.HTTP_201_CREATED, summary='Добавить новый термин')
def write_term(db: SessionDep, term: GlossaryTermUpdate):
...
def update_term(term_id: int, db: SessionDep, term: GlossaryTermUpdate):
